...
def substrings(string)
  ...
  (0..string.length).each do |i|
    (0..string.length).each do |j|
      final_str << string[i..j]
    end 
  end 
final_str
end

puts substrings("cat") # ["c", "ca", "cat", "a", "at",
# "t"]`.

def subwords(word, dictionary)
  dictionary.select do |string|
    substrings(word).include?(string)
  end 
end
...
def concatenate(strings)
  strings.inject{|arr, str| arr + str}
end
